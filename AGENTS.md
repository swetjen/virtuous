...
- `internal/adminui/html.go`: integrated docs/admin shell (Api and Observability modules).
...
- `ServeDocs()` enables all docs modules by default (`Api`, `Observability`).
...
- Use `WithDocsGuards(...)` for docs/OpenAPI protection and `WithAdminGuards(...)` for docs/admin protection.
...
- If the `Observability` module is enabled but runtime attachment is missing (`AttachLogger(...)`), docs should render a zero-state setup snippet rather than hard-failing.
